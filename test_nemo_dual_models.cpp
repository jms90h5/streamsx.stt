...
struct TestConfig {
    std::string encoder_model_path = "models/nemo_fresh_export/encoder-fastconformer_cache_final.onnx";
    std::string decoder_model_path = "models/nemo_fresh_export/decoder_joint-fastconformer_cache_final.onnx";
...
void printUsage(const char* program_name) {
    std::cout << "Usage: " << program_name << " [OPTIONS]\n"
              << "\nOptions:\n"
              << "  --encoder-model PATH  Path to encoder ONNX model file\n"
              << "  --decoder-model PATH  Path to decoder ONNX model file\n"
              << "  --audio-file PATH     Path to raw audio file (16kHz, 16-bit, mono)\n"
              << "  --output-file PATH    Path to output file for results (optional)\n"
              << "  --enable-cache        Enable cache-aware streaming (default: true)\n"
              << "  --disable-cache       Disable cache-aware streaming\n"
              << "  --chunk-size MS       Chunk size in milliseconds (default: 160)\n"
              << "  --verbose             Enable verbose output\n"
              << "  --help                Show this help message\n"
              << "\nExample:\n"
              << "  " << program_name << " --encoder-model models/encoder.onnx --decoder-model models/decoder.onnx --audio-file test.raw\n"
              << std::endl;
}
