...
: MY_BASE_OPERATOR(),
  _shutdown(false),
  _pattern((((lit$2 + lit$1) + ::SPL::spl_cast<SPL::rstring, SPL::float64 >::cast(::SPL::Functions::Time::getTimestampInSecs())) + lit$0)),
...
void MY_OPERATOR_SCOPE::MY_OPERATOR::process(Tuple const & tuple$, uint32_t port)
{
    ...
    try {
        ...
        SPL::serializeWithPrecision(_f->writeTo(), iport$0.get_filename()) << ',';
	SPL::serializeWithPrecision(_f->writeTo(), iport$0.get_transcription()) << ',';
	SPL::serializeWithPrecision(_f->writeTo(), iport$0.get_wordCount()) << ',';
	SPL::serializeWithPrecision(_f->writeTo(), iport$0.get_charCount()) << ',';
	SPL::serializeWithPrecision(_f->writeTo(), iport$0.get_audioDurationSec()) << ',';
	SPL::serializeWithPrecision(_f->writeTo(), iport$0.get_processingTimeSec()) << ',';
	SPL::serializeWithPrecision(_f->writeTo(), iport$0.get_speedupFactor()) << ',';
	SPL::serializeWithPrecision(_f->writeTo(), iport$0.get_startTime()) << ',';
	SPL::serializeWithPrecision(_f->writeTo(), iport$0.get_endTime()) << '\n';
...
bool MY_BASE_OPERATOR::globalIniter() {
    instantiators_.insert(std::make_pair("AnalysisCSV",&initer));
    return true;
}
...
MY_BASE_OPERATOR::MY_BASE_OPERATOR()
 : Operator()  {
    ...
    param$file$0 = (((lit$2 + lit$1) + ::SPL::spl_cast<SPL::rstring, SPL::float64 >::cast(::SPL::Functions::Time::getTimestampInSecs())) + lit$0);
